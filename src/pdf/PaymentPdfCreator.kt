...
class PaymentPdfCreator(private val pdfData: PaymentPdfData) :
    PdfCreator("${pdfData.title.capitalize()}-${pdfData.interval.first.year}") {
    companion object {
        ...
        private val MAPPER = { p: Payment ->
            listOf(
                DataCell(formatter.print(p.date)), DataCell("${p.amount} €"), DataCell(p.comment, commentFont)
            )
        }
    }
    ...
    override fun populate(document: Document) {
        ...
        document.add(status("Stanje ${df.print(pdfData.interval.first.minusDays(1))}: ${pdfData.balance} €"))
        ...
        document.add(status("Stanje ${df.print(pdfData.interval.second)}: ${pdfData.balance + sumAll} € ($sumAll €)", 20))
        val data: MutableList<List<DataCell>> = pdfData.payments.filter(pdfData.filter).map(MAPPER).toMutableList()
        if (data.isNotEmpty())
            data.add(
                mutableListOf(DataCell("Sum"), DataCell("$sumFiltered €"), DataCell(""))
            )
